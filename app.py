...
# ==================== CARGAR DATOS DESDE API CON PAGINACIÓN ====================
@st.cache_data(ttl=3600)
def cargar_datos():
    try:
        base_url = "https://www.datos.gov.co/resource/p6dx-8zbt.json"
        
        # Consulta limpia y mínima (solo columnas que usas)
        query = "SELECT departamento_entidad, ciudad_entidad, id_del_proceso, nombre_del_procedimiento, descripci_n_del_procedimiento, fecha_de_publicacion_del, valor_total_adjudicacion, urlproceso, estado_del_procedimiento"
        
        all_data = []
        offset = 0
        limit = 5000  # Tamaño seguro por página
        
        st.info("Cargando datos por páginas (puede tardar 1–3 minutos dependiendo del tamaño del dataset)...")
        
        progress_bar = st.progress(0)
        status_text = st.empty()
        
        while True:
            url = f"{base_url}?$query={urllib.parse.quote(query)}&$limit={limit}&$offset={offset}"
            status_text.text(f"Cargando página con offset {offset}...")
            
            try:
                page_df = pd.read_json(url)
                if page_df.empty:
                ...
                progress_bar.progress(min(offset / 100000, 1.0))  # Asumiendo max ~100k filas
            except Exception as e:
                st.error(f"Error en página offset {offset}: {str(e)}")
                break
        ...
        st.success(f"¡Éxito! Datos completos cargados: {len(df)} filas.")
        return df
